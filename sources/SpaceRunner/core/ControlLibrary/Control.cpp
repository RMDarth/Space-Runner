...
Control::Control(SceneSector* sector, std::string type, std::string name, float x, float y, float width, float height, std::string defaultImage, Control* parent, Control::RenderType renderType)
{
	_sector = sector;
	_type = type;
...
void Control::InitOverlay()
{
	...
	_overlayText->setPosition(0, -0.0375f);
 
    _overlayText->setParameter("font_name", "GUIfont");
	_overlayText->setParameter("char_height", "0.075");
	...
	_overlayTextShadow->setPosition(0.002f, -0.035f);
	_overlayTextShadow->setParameter("font_name", "GUIfont");
	_overlayTextShadow->setParameter("char_height", "0.075");
